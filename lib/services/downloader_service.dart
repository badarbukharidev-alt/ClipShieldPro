...
class DownloaderService {
  ...
  /// Fetches captions as structured cues with real start and end times.
  ///
  /// The string-formatted [fetchSubtitles] is kept for the AI highlight
  /// detector, which only needs a transcript. Burning captions in needs genuine
  /// end timings, which that format discards.
  Future<List<CaptionCue>> fetchCaptionCues(String url) async {
    final id = getVideoId(url);
    if (id == null) return const [];

    try {
      final manifest = await _yt.videos.closedCaptions.getManifest(id);
      if (manifest.tracks.isEmpty) return const [];

      ClosedCaptionTrackInfo selected;
      final english = manifest.tracks
          .where((t) => t.language.code.toLowerCase().startsWith('en'))
          .toList();
      if (english.isNotEmpty) {
        // Prefer human-written captions; auto-generated timing is looser.
        selected = english.firstWhere(
          (t) => !t.isAutoGenerated,
          orElse: () => english.first,
        );
      } else {
        selected = manifest.tracks.first;
      }

      final track = await _yt.videos.closedCaptions.get(selected);

      final cues = <CaptionCue>[];
      for (final caption in track.captions) {
        final text = caption.text.replaceAll(RegExp(r'\s+'), ' ').trim();
        if (text.isEmpty) continue;

        final start = caption.offset.inMilliseconds / 1000.0;
        final end = (caption.offset + caption.duration).inMilliseconds / 1000.0;
        if (end <= start) continue;

        cues.add(CaptionCue(startSeconds: start, endSeconds: end, text: text));
      }
      return cues;
    } catch (_) {
      return const [];
